...
class FindServer(Node):
    def __init__(self):
        ...
        self.server = self.create_service(Boxpoint, 
                              'find_server', self.callback_service)
    ...
    def segment_box(self, color_image, depth_image):
        ...
        for i in range(len(masks)):
            if np.count_nonzero(masks[i]["segmentation"]) < 5000 and np.count_nonzero(masks[i]["segmentation"]) > 2000:
                mask_list.append(i)
        ...
        if box_mask_index is None:
            ...
            self.segment_box( color_image, depth_image) # 다시 실행
        ...
        try:
            ...
            cv2.circle(box_depth, (int(x), int(y)), 10, 255, -1)
